...
gcc -o mprod -fopenmp matrix-product-parallel.c -lm
*/
{
  double a[500][500];
  double angle;
  ...
  timestamp ( );//new! calling a C function.
...
# pragma omp parallel shared ( a, b, c, n, pi, s ) private ( angle, i, j, k )
{
  ...
  for ( i = 0; i < n; i++ )
  {
    ...
    printf(" Matrix a calculation: at index %d using thread %d. \n", i,  omp_get_thread_num());
...
*/
  ...
  for ( i = 0; i < n; i++ )
  {
    ...
    printf("Matrix b calculation: at index %d using thread %d. \n", i,  omp_get_thread_num());
